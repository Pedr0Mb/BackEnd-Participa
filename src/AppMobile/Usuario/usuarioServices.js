  ...
  export async function criarUsuario(data) {
    ...
    if (!cpfQuery.empty) throw Object.assign(new Error("Usuário com o mesmo CPF já cadastrado"), { status: 409 })

    const emailQuery = await usuarioRef.where('email', '==', data.email).get();

    if (!emailQuery.empty) throw Object.assign(new Error("E-mail já cadastrado"), { status: 409 });
