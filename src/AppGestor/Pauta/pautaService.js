...
export async function editarPauta(data) {
  ...
  if (doc.data().status !== 'rascunho') 
    throw Object.assign(new Error('Pauta publicada não pode ser editada'), { status: 400 })
...
export async function publicarPauta(data) {
  ...
  if (doc.data().status !== 'rascunho') 
    throw Object.assign(new Error('Pauta publicada não pode ser publicada novamente'), { status: 400 })
